...
import { Menu, Building2, LayoutDashboard, DoorOpen, Wrench, CalendarClock, CreditCard, Users2, Settings } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center gap-4">
        <button className="sm:hidden" onClick={() => setOpen((s) => !s)} aria-label="Toggle menu">
        ...
        <nav className={cn("hidden sm:flex items-center gap-1 ml-4", open && "flex")}> 
          {nav.map((item) => {
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    (isActive || pathname.startsWith(item.to)) && "bg-accent/40 text-foreground"
                  )
                }
